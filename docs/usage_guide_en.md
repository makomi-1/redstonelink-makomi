...
- Known boundary: if a `force-load` chunk activator becomes active only after the real `sync` change already happened, and both source and target are already offline by then, the target chunk may still be brought up to `online=true` while the high state is not restored. This boundary is concentrated in the combination "late `force-load` activator + source already soft-offline + recovery depends on a historical `sync` snapshot"; it does not affect the normal mainline where a player directly loads the target chunk or where `resident` keeps the target online first.
- Recommended mitigation: for that class of scenario, prefer enabling `crosschunk.syncSignalPersistent=true` as the complete `sync` fallback. If you do not want it globally, override it only in the relevant dedicated bench / suite or the deployment profile that needs it.
...
11a. For the known boundary "late `force-load` chunk activator while both source and target were already offline", directly enabling `crosschunk.syncSignalPersistent=true` is the recommended mitigation; this scenario should not rely only on the default finite-TTL historical `sync` snapshot recovery
