...
- `sync`：状态信号；同 tick 内按来源强度聚合并取 `max`，跨 tick 只保留最新一帧，同时保留该帧内并列最大来源列表供观测；它享有来源级失效、补发与 replay 服务，也是默认与推荐的红石机器主链路。
...
### 8.6 智能眼镜与 visualize 观察门槛

`Quick Link Tool` 的 `visualize` 已不再以“手持 QLT”作为显示依托，观察入口改由 `Smart Glasses` 承接：

- 佩戴智能眼镜后，远/近外显与第三形态连线显示才允许出现；这是一层客户端观察门槛，不是新的拓扑真值。
- `visualize` 的增删显示对象与 `Shift + B` 清空仍属于操作路径，因此继续要求主手为空；其中增删对象额外要求站立并按住 `Ctrl`，但显示本身不要求空手，方便玩家拿着其它工具持续观察。
- 已加入的显示对象会在客户端保留本地快照，并按 `sourceRevision/coreRevision + runtimeNodeVersion` 做批量增量刷新。
- 对准 tooltip 与穿墙连线只读取本地缓存，不做逐帧网络查询；真正的网络开销集中在“对象新增”与“增量刷新”两个批次上。

### 8.7 智能节点容器的物品内库存语义

`Smart Node Container` 是独立的物品内库存系统，不是旧聚合栈的 GUI 包装：

- 它存的是完整 `ItemStack`，不是纯 serial group；节点物品侧状态会跟随容器一起保存。
- 容器只接受 `core/triggerSource/repeater` 三类节点，并额外持久化“当前放置类型”“自动排序开关”与外显状态。
- 主手右键放置时，容器按“当前放置类型”弹出首个匹配物品尝试放置；自动排序只影响库存组织，不改变节点真值。
- 若容器物品实体被真正销毁，内部节点不会先掉出，而是递归走现有退役协调流程，避免留下“已分配但无载体”的幽灵序号。
