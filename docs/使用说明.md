...
- 建议先读：`快速连接工具` -> `同步遥控器` -> `状态面板工具` -> `命令/GUI 批量序号输入格式`。
...
3. 将 `core` 缓存应用到 `triggerSource` 时，会把该 `triggerSource` 的目标集合整体覆盖为缓存中的 `core` 集合。
4. 将 `triggerSource` 缓存应用到 `core` 时，会把缓存里的每个 `triggerSource` 覆盖为“仅连接当前命中的这个 `core`”。
5. 当前序号缓存为空时，右键应用会直接失败并提示。
6. 将一批 `triggerSource` 缓存应用到 `core` 时，缓存项数同样受服务端 `server.maxTargetsPerSetLinks` 限制。
7. 快速连接工具的应用同样受服务端写控限制；若命中只读、limited 或受控名单限制，当前统一提示“没有足够权限”。
- 清空规则：
1. 中键清空只清空序号缓存与频道缓存。
...
4. 真正执行应用时，服务端仍会按 `server.command.linkSet.maxInputLength` 对缓存表达式再做一次长度校验。
5. 手持快速连接工具并命中可连接节点时会显示描边：`core` 为亮蓝色，`triggerSource` 为亮橙色。
- 当前建议使用顺序：
1. 主手持快速连接工具。
...
- 不同类型使用固定颜色区分（`core` 为青色，`triggerSource` 为橙色；当前连接核心片显示为 `core` 颜色）。
