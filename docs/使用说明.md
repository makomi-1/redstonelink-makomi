...
- 建议先读：`快速连接工具` -> `发送/接收过滤器` -> `同步遥控器` -> `状态面板工具` -> `命令/GUI 批量序号输入格式`。
...
7. 当前序号缓存为空时，右键应用会直接失败并提示。
8. 将一批 `triggerSource` 缓存应用到 `core` 时，缓存项数同样受服务端 `server.maxTargetsPerSetLinks` 限制。
9. 命中 `send` 过滤器时，只接受 `triggerSource` 缓存，并仅覆盖过滤器的 `serialExpression`；命中 `receive` 过滤器时，只接受 `core` 缓存。
10. 过滤器应用不参与 OCC；基线往返仍会执行，但服务端会回传零 revision 并直接写入过滤器配置。
- 清空规则：
1. 中键清空只清空序号缓存与频道缓存。
...
5. 手持快速连接工具并命中对象时会显示描边：`core` 为亮蓝色，`triggerSource` 为亮橙色，过滤器为亮红色。
- 当前建议使用顺序：
1. 主手持快速连接工具。
...
1. 放置后以方块形式持续生效。
2. 潜行右键打开过滤器配置界面。
3. 手持快速连接工具并站立右键命中过滤器时，可把对应类型的序号缓存直接应用到过滤器的节点输入框。
- 服务对象：
1. `send` 过滤器只服务 `triggerSource`。
2. `receive` 过滤器只服务 `core`。
3. 过滤器不会改变真实写入方向，链路仍固定为 `triggerSource -> core`。
- 配置项：
1. `serialExpression`：节点集输入，支持与配对 GUI 相同的 `N` / `A:B` 语法。
2. `nodeSetMode`：`disabled / whitelist / blocklist`，分别表示关闭、白名单、黑名单筛选。
3. `signalThresholdSource`：`fixed_input / neighbor_max_input`，分别表示固定阈值和邻居最大输入。
4. `fixedSignalThreshold`：固定阈值输入，范围 `0~15`。
5. `signalMode`：`disabled / upper_bound / lower_bound`，分别表示关闭、上界过滤、下界过滤。
- 作用范围与运行时：
1. 物理作用范围是以过滤器方块为中心、`X/Y/Z` 各方向半径 `8` 格的正方体。
2. 只有作用范围内且命中节点集/信号条件的节点会被过滤。
3. 过滤器配置变化或邻居输入变化后，会立即重采样并刷新运行态。
4. 对 `sync` 链路而言，过滤器从放行变为拦截时会触发失效；从拦截变为放行时会按当前快照补发。
5. `pulse / toggle` 不做历史补发，只影响后续新派发。
