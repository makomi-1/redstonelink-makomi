...
3. 曲线页与 graph 页共用同一套本地网页桥，当前网页主入口只承载 `graph/recording` 两个独立页面。
...
3. `Save` 之前会先走保存预检；本地草稿、布局和导出的图快照都保存在客户端本地网页资产目录中。
4. 网页支持中英文切换与主题切换；当前偏好会持久化到 `gameDir/redstonelink/web/preferences.json`，重启游戏后仍会继续使用。
- 导出与缓存说明：
