...
- 当前版本实际可用工作流仍围绕“序号缓存”展开；`channel` 模式现可切换并编辑缓存，但 collect/apply 仍只提示“未来扩展”，不参与真实建链；其 GUI 背景与物品贴图会切到独立频道主题，便于和 `serial` 直接区分。
- 基础交互：
...
3. 当前应用编辑模式分三种：`replace`（覆盖）、`append`（增量增加）、`remove`（增量删除）。
...
7. 当前序号缓存为空时，仅 `replace` 可继续应用，语义为“覆盖为空”：
   - 命中 `triggerSource` 时清空当前一跳 `core` 集合
   - 命中 `core` 时清空当前一跳 `triggerSource` 集合
   - 命中过滤器时清空当前节点集
8. `append/remove` 仍要求缓存非空；空缓存下会直接失败并提示。
9. 将一批 `triggerSource` 缓存应用到 `core` 时，缓存项数同样受服务端 `server.maxTargetsPerSetLinks` 限制。
10. 命中 `send` 过滤器时，只接受 `triggerSource` 缓存；命中 `receive` 过滤器时，只接受 `core` 缓存。过滤器的 `serialExpression` 同样支持 `replace/append/remove` 三态编辑。
11. 过滤器应用不参与 OCC；基线往返仍会执行，但服务端会回传零 revision 并直接写入过滤器配置。
- 清空规则：
1. `潜行 + B` 清空序号缓存与频道缓存。
2. 清空后会保留当前模式、当前序号缓存类型与当前应用编辑模式，不会强制重置为默认值。
- 界面与反馈：
1. GUI 中可手动编辑序号缓存；切到 `channel` 后也可编辑频道缓存，但当前不参与真实 collect/apply。`serial/channel` 会分别使用 `_sd/_cp` 物品贴图和对应主题背景。
...
3. 用中键切到需要的应用编辑模式。
4. 站立右键把缓存应用到合法方向的目标节点，或把对应类型缓存应用到过滤器。
5. 需要重来时按 `潜行 + B` 清空缓存。
- 合成配方：
1. `连接原件 + 木棍 + 木棍 -> 快速连接工具`
2. 图形如下：
...
- 近距离准星命中节点时，可在屏幕中心显示五行信息（深色背景），仅在近距离（8 格内）生效：
1. `[物品名]序号`
2. 激活状态（`ON` / `OFF`）
3. 最终 IO（`I/O`）
