...
### 隐藏节点
- 物品名：`隐藏核心块 / Hidden Core`、`隐藏同步 triggerSource / Hidden Sync triggerSource`。
- 基础行为：
1. `隐藏核心块` 作为隐藏版 `core` 使用，`隐藏同步 triggerSource` 作为隐藏版状态型 `triggerSource` 使用。
2. 未佩戴智能眼镜时，它们默认不可见，也不能像普通节点那样正常选中和交互。
3. 佩戴智能眼镜后，才可查看、编辑、回收和进行其它正常操作。
- 定向行为：
1. `隐藏核心块` 只会向启用面输出。
2. `隐藏同步 triggerSource` 只会从启用面采样输入。
3. 两者放置后默认面集为空，需要先用定向面编辑工具启用需要的面。
- 合成配方：
1. `连接核心块 + 玻璃 -> 隐藏核心块`
...
- 当前支持：
1. `core` 与 `hide core`
2. 块状 `triggerSource` 与 `hide sync triggerSource`
3. `repeater`
4. `send/receive` 过滤器
5. 区块激活器
- 默认规则：
...
3. 转发器会把输入侧观察到的状态按固定 `1/2 tick` 延迟转发到输出侧 `core`，适合作为缓冲与定长延迟节点。
