...
- 建议先读：`快速连接工具` -> `发送/接收过滤器` -> `同步遥控器` -> `转发器` -> `状态面板工具` -> `图可视化编辑器` -> `命令/GUI 批量序号输入格式`。

### 快速连接工具
- 物品名：`快速连接工具 / Quick Link Tool`。
- 当前版本提供 `serial/channel/visualize` 三种模式：`serial` 用于序号缓存批量采集/应用，`channel` 用于频道缓存批量采集/应用，`visualize` 用于把节点或转发器加入显示对象列表并查看穿墙连线。三种模式会分别切到 `_sd/_cp/_vs` 物品贴图与对应主题。
- 基础交互：
1. 潜行且左手为空时右键：在 `serial/channel` 下打开快速连接工具缓存编辑界面；`visualize` 下不会打开编辑器。
2. 左键命中可连接节点：`serial` 采集当前节点序号到缓存；`channel` 采集当前节点频道到缓存；`visualize` 把当前节点或转发器加入显示对象并查询其真实连接。
3. 站立右键命中可连接节点或对应过滤器：`serial/channel` 把当前缓存应用到命中对象；`visualize` 从显示对象列表中移除该对象。
4. 鼠标中键（跟随原版 `pick item` 绑定）：仅在 `serial` 模式下循环切换应用编辑模式 `覆盖 -> 增量增加 -> 增量删除 -> 覆盖`。
5. 快速连接模式键：默认 `B`；站立按切换 `serial/channel/visualize`，潜行按清空当前模式数据；`visualize` 下会清空全部显示对象。
- 采集规则：
...
- 清空规则：
1. `潜行 + B` 清空当前模式数据；`serial` 清空序号缓存，`channel` 清空频道缓存，`visualize` 清空显示对象列表。
...
- 当前建议使用顺序：
1. 主手持快速连接工具。
2. 按 `B` 选择当前是 `serial`、`channel` 还是 `visualize` 工作流。
3. `serial/channel` 下左键采集目标缓存，`visualize` 下左键添加显示对象。
4. 若当前使用 `serial`，可再用中键切到需要的应用编辑模式。
5. `serial/channel` 下站立右键把缓存应用到合法方向的目标节点或过滤器；`visualize` 下站立右键移除显示对象。
6. 需要重来时按 `潜行 + B` 清空当前模式数据。
- 合成配方：
1. `连接原件 + 木棍 + 木棍 -> 快速连接工具`
2. 图形如下：
   `  C`
