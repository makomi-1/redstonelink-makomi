...
4. `visualize` 的左键添加、右键移除与 `Shift + B` 清空仍要求主手为空。
- 行为说明：
...
2. 左键命中可连接节点：`serial` 采集当前节点序号到缓存；`channel` 采集当前节点频道到缓存；`visualize` 只有在“已佩戴智能眼镜且主手为空”时，才会把当前节点或转发器加入显示对象并查询其真实连接。
3. 站立右键命中可连接节点或对应过滤器：`serial/channel` 把当前缓存应用到命中对象；`visualize` 只有在“已佩戴智能眼镜且主手为空”时，才会从显示对象列表中移除该对象。
4. 鼠标中键（跟随原版 `pick item` 绑定）：仅在 `serial` 模式下循环切换应用编辑模式 `覆盖 -> 增量增加 -> 增量删除 -> 覆盖`。
5. 快速连接模式键：默认 `B`；站立按切换 `serial/channel/visualize`，潜行按清空当前模式数据；`visualize` 下会在“已佩戴智能眼镜且主手为空”时清空全部显示对象。
- 采集规则：
...
- 清空规则：
1. `潜行 + B` 清空当前模式数据；`serial` 清空序号缓存，`channel` 清空频道缓存，`visualize` 会在“已佩戴智能眼镜且主手为空”时清空显示对象列表。
2. 清空后会保留当前模式、当前序号缓存类型与当前应用编辑模式，不会强制重置为默认值。
- 界面与反馈：
1. GUI 中可手动编辑序号缓存；切到 `channel` 后也可直接编辑频道缓存，并参与真实 collect/apply。`visualize` 不打开缓存编辑器，而是完全使用左键/右键/潜行模式键管理显示对象。`serial/channel/visualize` 会分别使用 `_sd/_cp/_vs` 物品贴图和对应主题背景。
...
4. `serial/channel` 下左键采集目标缓存；`visualize` 下切到主手为空后，再左键添加显示对象。
5. 若当前使用 `serial`，可再用中键切到需要的应用编辑模式。
6. `serial/channel` 下站立右键把缓存应用到合法方向的目标节点或过滤器；`visualize` 下切到主手为空后，再右键移除显示对象。
7. `visualize` 对象加入后，即使重新拿起其它工具，只要仍佩戴智能眼镜，也能继续查看现有连线。
8. 需要重来时，在佩戴智能眼镜且主手为空的前提下按 `潜行 + B` 清空显示对象。
- 合成配方：
1. `连接原件 + 木棍 + 木棍 -> 快速连接工具`
2. 图形如下：
   `  C`
