...
- 建议先读：`智能眼镜` -> `隐藏节点` -> `定向面编辑工具` -> `快速连接工具` -> `智能节点容器` -> `发送/接收过滤器` -> `同步遥控器` -> `转发器` -> `状态面板工具` -> `图可视化编辑器` -> `命令/GUI 批量序号输入格式`。

### 智能眼镜
- 物品名：`智能眼镜 / Smart Glasses`。
- 佩戴作用：
1. 佩戴后启用远/近外显。
2. 佩戴后才允许显示 `visualize` 的穿墙连线与对准 tooltip。
3. `hide` 节点只有在佩戴后才可见、可正常选中并交互。
4. 只要佩戴了眼镜，就算主手拿着快速连接工具或其它工具也能继续查看已显示连线。
5. `visualize` 的左键添加、右键移除与 `Shift + B` 清空仍要求主手为空；其中添加/移除还要求站立并按住 `Ctrl`。
6. 默认 `Ctrl + K` 可切换是否持续显示定向方向箭头；该显示条件会与定向面编辑工具取并集。
- 行为说明：
1. 智能眼镜只承担观察入口，不负责配对、改链或缓存编辑。
2. 未佩戴时，远/近外显与 `visualize` 显示都不会生效。
3. 当前复用头盔装备槽与原版护甲渲染链，不提供额外护甲属性，也没有普通头盔耐久。
4. 方向箭头持续显示是纯客户端观察开关，不会改变任何 `triggerSource/core` 真值或节点配置。
- 合成配方：
1. `玻璃板 + 连接原件 + 玻璃板 -> 智能眼镜`
2. 图形如下：
   `GLG`
...
### 隐藏节点
- 物品名：`隐藏核心块 / Hidden Core`、`隐藏同步 triggerSource / Hidden Sync triggerSource`、`隐藏切换发射器 / Hidden Toggle Emitter`、`隐藏脉冲发射器 / Hidden Pulse Emitter`、`隐藏发送过滤器 / Hidden Send Filter`、`隐藏接收过滤器 / Hidden Receive Filter`、`隐藏区块激活器 / Hidden Chunk Activator`、`隐藏转发器 / Hidden Repeater`。
- 基础行为：
1. 这些物品分别作为对应可见块状节点的隐藏版载体使用，不改变原有 `triggerSource/core` 语义，只改变可见性与交互门槛。
2. 未佩戴智能眼镜时，它们默认不可见，也不能像普通节点那样正常选中和交互。
3. 佩戴智能眼镜后，才可查看、编辑、回收和进行其它正常操作。
- 定向行为：
1. `隐藏核心块` 只会向启用面输出。
2. 隐藏版块状 `triggerSource`、隐藏过滤器、隐藏区块激活器与隐藏转发器会沿用对应可见节点的定向输入/输出语义。
3. 所有 `hide` 块状节点放置后默认面集为空，需要先用定向面编辑工具启用需要的面。
- 合成配方：
1. `连接核心块 + 玻璃 -> 隐藏核心块`
2. `同步发射器 + 玻璃 -> 隐藏同步 triggerSource`
3. 其它 `hide` 块状节点也统一使用“原节点 + 玻璃”的任意摆放配方。

### 定向面编辑工具
- 物品名：`定向面编辑工具 / Directional Face Editor`。
- 基础交互：
1. 鼠标中键可循环切换 `toggle / single / all / clear` 四种模式。
2. 主手右键命中支持节点的某一面时，会按当前模式编辑该节点的面集。
3. 副手拿着该工具时只显示箭头，不会真正修改节点。
4. 佩戴智能眼镜后，只要主手或副手持有该工具，就能看到节点当前启用面的方向箭头。
5. 若智能眼镜的 `Ctrl + K` 方向显示开关已开启，即使当前没拿定向面编辑工具，也会继续显示这些箭头。
- 当前支持：
1. `core` 与 `hide core`
2. 块状 `triggerSource` 与对应的隐藏块状 `triggerSource`
3. `repeater`
4. `send/receive` 过滤器
5. 区块激活器
6. 上述各类的隐藏节点变体
...
- 当前版本提供 `serial/channel/visualize` 三种模式：`serial` 用于序号缓存批量采集/应用，`channel` 用于频道缓存批量采集/应用，`visualize` 用于把节点或转发器加入显示对象列表并查看穿墙连线；其中显示入口由智能眼镜承接。三种模式会分别切到 `_sd/_cp/_vs` 物品贴图与对应主题。
- 基础交互：
1. 潜行且左手为空时右键：在 `serial/channel` 下打开快速连接工具缓存编辑界面；`visualize` 下不会打开编辑器。
2. 左键命中可连接节点：`serial` 采集当前节点序号到缓存；`channel` 采集当前节点频道到缓存；`visualize` 只有在“已佩戴智能眼镜、主手为空、站立且按住 `Ctrl`”时，才会把当前节点或转发器加入显示对象并查询其真实连接。
3. 站立右键命中可连接节点或对应过滤器：`serial/channel` 把当前缓存应用到命中对象；`visualize` 只有在“已佩戴智能眼镜、主手为空、站立且按住 `Ctrl`”时，才会从显示对象列表中移除该对象。
4. 鼠标中键（跟随原版 `pick item` 绑定）：仅在 `serial` 模式下循环切换应用编辑模式 `覆盖 -> 增量增加 -> 增量删除 -> 覆盖`。
5. 快速连接模式键：默认 `B`；站立按切换 `serial/channel/visualize`，潜行按清空当前模式数据；`visualize` 下会在“已佩戴智能眼镜且主手为空”时清空全部显示对象，该操作不要求按住 `Ctrl`。
- 采集规则：
...
- 清空规则：
1. `潜行 + B` 清空当前模式数据；`serial` 清空序号缓存，`channel` 清空频道缓存，`visualize` 会在“已佩戴智能眼镜且主手为空”时清空显示对象列表，该操作不要求按住 `Ctrl`。
2. 清空后会保留当前模式、当前序号缓存类型与当前应用编辑模式，不会强制重置为默认值。
- 界面与反馈：
1. GUI 中可手动编辑序号缓存；切到 `channel` 后也可直接编辑频道缓存，并参与真实 collect/apply。`visualize` 不打开缓存编辑器，而是完全使用按住 `Ctrl` 的左键/右键与潜行模式键管理显示对象。`serial/channel/visualize` 会分别使用 `_sd/_cp/_vs` 物品贴图和对应主题背景。
...
- 当前建议使用顺序：
1. 主手持快速连接工具。
2. 若要使用 `visualize`，先佩戴智能眼镜。
3. 按 `B` 选择当前是 `serial`、`channel` 还是 `visualize` 工作流。
4. `serial/channel` 下左键采集目标缓存；`visualize` 下切到主手为空并按住 `Ctrl` 后，再左键添加显示对象。
5. 若当前使用 `serial`，可再用中键切到需要的应用编辑模式。
6. `serial/channel` 下站立右键把缓存应用到合法方向的目标节点或过滤器；`visualize` 下切到主手为空并按住 `Ctrl` 后，再右键移除显示对象。
7. `visualize` 对象加入后，即使重新拿起其它工具，只要仍佩戴智能眼镜，也能继续查看现有连线。
8. 需要重来时，在佩戴智能眼镜且主手为空的前提下按 `潜行 + B` 清空显示对象；该操作不要求按住 `Ctrl`。
- 合成配方：
1. `连接原件 + 木棍 + 木棍 -> 快速连接工具`
2. 图形如下：
   `  C`
...
### 智能节点容器
- 物品名：`智能节点容器 / Smart Node Container`。
- 基础交互：
1. 主手持有时按快速连接模式键（默认 `B`）打开 `6` 行、共 `54` 格的箱子式 GUI。
2. GUI 右侧显示“当前放置类型”“自动排序”，以及创造模式下的“自动消耗”开关；容器槽位与背包槽位都可像普通箱子一样悬停查看 tooltip。
3. 鼠标中键循环切换当前放置类型：`core -> triggerSource -> repeater`。
4. 主手按住 `Ctrl + 鼠标滚轮` 可在当前放置类型内临时选取某个已排序节点；该选取只对下一次成功作用生效，随后自动回到默认顺序，除非再次主动选取。
5. 主手右键会按当前放置类型弹出一个节点并尝试直接放置；若刚做了临时选取，则优先使用该节点。
5. 潜行主手右键可把可见或隐藏节点受控回收到容器；容器满时回退为掉落物。
- 存储规则：
1. 仅接受未聚合的单个节点物品：`core`、`triggerSource` 与 `repeater`，其中也包含隐藏节点变体。
2. 容器保存的是完整物品栈快照，节点自身的序号、频道与其它物品侧状态会一并保留。
3. 自动排序开启时，按 `core -> triggerSource -> repeater` 排序，类内再按序号升序。
4. 容器非空时，侧面外显跟随当前选中的放置类型切换。
5. 节点放在其中时，连接信息 tooltip 不会随外部变化自动同步，这一点与原版箱子一致。
6. 若容器物品实体被真正销毁，内部节点会递归退役，不会先掉出来。
7. 创造模式下可切换“自动消耗”；开启时放置成功会真实消耗容器内节点，关闭时不会消耗。
8. `hide repeater` 现在会和可见转发器一样，被正确识别为 `repeater` 放置类型。
- 合成配方：
1. `箱子 + 连接原件 -> 智能节点容器`
2. 图形如下：
   `CL `
...
- 基础交互与其它遥控器一致：
1. 潜行且左手为空时右键：打开配对界面。
2. 站立且左手为空时右键：向已连接 `core` 发送一次当前设定同步强度。
- 行为说明：
1. 同步遥控器固定作为 `triggerSource` 使用，真实写入方向仍为 `triggerSource -> core`。
2. 它走的是“同步拉杆”同链路，同步的是当前强度，不是 `TOGGLE/PULSE` 激活语义。
3. 当前强度范围为 `0~15`；可在配对 GUI 中用增减按钮调整，也支持手持时按住 `Ctrl + 鼠标滚轮` 快速修改，调整时会在 action bar 显示当前强度。
4. 配对 GUI 当前强度区域中间只显示纯数字，不再显示“当前强度”字段名。
5. 物品贴图仍按 `0/非 0` 二态切换：`0` 态显示关闭贴图，任意非 `0` 强度显示开启贴图。
- 合成配方：
1. `同步拉杆 + 拉杆 -> 同步遥控器`
2. 图形与其它遥控器一致：
   `BL`
...
### 转发器
- 物品名：`转发器 / Link Repeater`。
- 基础交互：
1. 放置后作为转发节点使用；潜行且左手为空时右键：打开转发器编辑器。
2. 编辑器分为输入 `triggerSource` 与输出 `core` 两侧，分别按当前图真值编辑。
3. 固定延迟现在支持任意大于 `0` 的正整数 tick；`2 tick` 以上属于实验性配置。
- 运行语义：
1. 转发器使用统一序号，同时承载 `triggerSource/core` 双身份；对外真实方向仍保持 `triggerSource -> core`。
2. 输入侧表达“哪些 triggerSource 指向当前转发器”，输出侧表达“当前转发器指向哪些 core”。
3. 转发器会把输入侧观察到的状态按当前配置的正整数 tick 延迟转发到输出侧 `core`，适合作为缓冲与定长延迟节点。
4. 转发器属于状态信号链路，享有完整生命周期和跨区块支持。
5. 转发器内部自连已自动隔离，不会保留同号 `triggerSource -> core` 非法自连。
6. 若延迟期间又来了新输入，会丢弃旧待派发结果，按新输入重新计时。
- 合成配方：
1. `连接核心块 + 连接原件 + 同步发射器 -> 转发器`
2. 图形如下：
   `CLS`
   `   `
   `   `
...
- `sync` 在同 tick 类内按强度聚合（取 `max`），跨 tick 仅保留最新一帧；`pulse/toggle` 共用一个事件域，同 tick 只保留一个事件结果，其中 `pulse` 高于 `toggle`。
