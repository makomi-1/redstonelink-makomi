...
- 建议先读：`智能眼镜` -> `快速连接工具` -> `智能节点容器` -> `发送/接收过滤器` -> `同步遥控器` -> `转发器` -> `状态面板工具` -> `图可视化编辑器` -> `命令/GUI 批量序号输入格式`。
...
3. 只要佩戴了眼镜，就算主手拿着快速连接工具或其它工具也能继续查看已显示连线。
4. `visualize` 的左键添加、右键移除与 `Shift + B` 清空仍要求主手为空；其中添加/移除还要求站立并按住 `Ctrl`。
- 行为说明：
...
2. GUI 右侧显示“当前放置类型”和“自动排序”；容器槽位与背包槽位都可像普通箱子一样悬停查看 tooltip。
3. 鼠标中键循环切换当前放置类型：`core -> triggerSource -> repeater`。
4. 主手右键会按当前放置类型弹出一个节点并尝试直接放置。
- 存储规则：
1. 仅接受未聚合的单个节点物品：`core`、`triggerSource` 与 `repeater`。
