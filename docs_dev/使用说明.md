...
9. 对 `sync` 与 `toggle` 来说，当前 `trace_read_tick_assert` 里的主要偏差是“观测平移”：以 `input start` 为锚点时，source/target 常从 `jobStartTick + 1` 才出现首个可见样本。这表示当前 trace 采样相位偏后，不等于运行时真实生效也晚了 1 tick。
