...
3. `functional-phase2.json`：第二期功能覆盖，补命令边界、resident 离线恢复、输入 job 重启语义、跨维度 sync、`syncTargetChunkLoadReplay` 与 `forceLoad.mode` 组合。
...
10. `nightly-full.json`：夜间全量，覆盖当前全部功能、跨区块/重启与纯性能基线场景；物理世界端到端场景单独运行。
...
8. `crosschunk.activation.pulse.relay.enabled`：是否启用 `pulse` 的普通 TTL relay（默认 `false`）。
9. `crosschunk.activation.pulse.ttlTicks`：`pulse` 普通 relay 的 TTL（tick）。
10. `crosschunk.activation.pulse.persistentExperimental`：是否启用 `pulse` 实验性不限时投递（默认 `false`）。
11. `crosschunk.activation.toggle.relay.enabled`：是否启用 `toggle` 的普通 TTL relay（默认 `false`）。
12. `crosschunk.activation.toggle.ttlTicks`：`toggle` 普通 relay 的 TTL（tick）。
13. `crosschunk.activation.toggle.persistentExperimental`：是否启用 `toggle` 实验性不限时投递（默认 `false`）。
14. `crosschunk.triggerSourceChunkUnloadInvalidation.enabled`：是否启用“triggerSource 区块卸载失效”，仅剔除目标上的 `sync` 贡献（默认 `false`）。
15. `crosschunk.triggerSourceInvalidation.enabled`：是否启用“triggerSource 其它失效”，剔除目标上的 `toggle/pulse/sync` 贡献（默认 `true`）。
16. `crosschunk.forceLoad.enabled`：强制加载总开关。
17. `crosschunk.forceLoad.mode`：`all` / `whitelist`。
...
9. `sync` 默认按“最新状态”不限时持久化（`crosschunk.syncSignalPersistent=true`），且目标区块 `CHUNK_LOAD` 时默认按来源端最近一次真实 sync 时间键补发。
...
11. 关闭 `crosschunk.syncTargetChunkLoadReplay.enabled` 后，才会完全跳过这条 `CHUNK_LOAD` 补发路径。
12. `pulse` 默认不做跨区块 relay；打开 `crosschunk.activation.pulse.relay.enabled=true` 后，才会按 TTL 缓冲；打开 `persistentExperimental=true` 后，可无限期等待目标加载后补发一次脉冲。
13. `toggle` 默认不做跨区块 relay；打开 `crosschunk.activation.toggle.relay.enabled=true` 后，才会按 TTL 缓冲；打开 `persistentExperimental=true` 后，可无限期等待目标加载后按净奇偶补发。
14. `triggerSource` 区块卸载事件只影响 `sync`，且默认关闭；开启 `crosschunk.triggerSourceChunkUnloadInvalidation.enabled=true` 后才会剔除该来源在目标上的 `sync` 贡献并重算。
15. `triggerSource` 离线/解绑/退役/删除属于“其它失效”事件，默认开启；命中后会剔除该来源在目标上的 `toggle/pulse/sync` 贡献并重算。
