...
5. 命中节点时，快速连接工具的应用仍受服务端写控限制；若命中只读、limited 或受控名单限制，当前统一提示“没有足够权限”。
6. 命中过滤器时，不走链接写控，改为要求服务端 `server.command.permissionLevel`。
7. 站立右键现在会先向服务端请求一次目标对象的 revision 基线，再自动继续正式 apply；用户侧仍表现为一次点击。
8. 命中 `triggerSource` 时，正式 apply 会校验该来源自己的 `sourceRevision`；命中 `core` 时，会校验该目标自己的 `coreRevision`。
9. 若预检与正式 apply 之间目标连接已被其他操作修改，本次 apply 会被拒绝并返回版本冲突提示，不会静默覆盖。
10. 命中 `send` 过滤器时，只接受 `triggerSource` 缓存，并仅覆盖过滤器的 `serialExpression`；命中 `receive` 过滤器时，只接受 `core` 缓存。
11. 过滤器应用不参与 OCC；基线往返仍会执行，但服务端会回传零 revision 并直接写入过滤器配置。
- 清空规则：
1. 中键清空只清空序号缓存与频道缓存。
...
4. 手持快速连接工具并命中对象时会显示描边：`core` 为亮蓝色，`triggerSource` 为亮橙色，过滤器为亮红色。
- 当前建议使用顺序：
1. 主手持快速连接工具。
2. 左键采集一批 `core` 或 `triggerSource` 到缓存。
3. 站立右键把缓存应用到合法方向的目标节点，或把对应类型缓存应用到过滤器。
4. 需要重来时按中键清空缓存。
- 合成配方：
...
```
11. 若要覆盖 dedicated server 行为，可显式传 `-ServerWindowMode Normal|Minimized|Hidden` 与 `-ServerPriorityClass Idle|BelowNormal|Normal|AboveNormal|High|RealTime`；当前 suite 入口默认 `ServerWindowMode=Hidden`。优先级方面，原始 `run-bench-suite.ps1` 仍不主动请求优先级，性能/必跑包装脚本默认 `ServerPriorityClass=High`。
- 性能 suite 用例列表与分类：
