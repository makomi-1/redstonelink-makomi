...
3. 运行单个场景：`powershell -ExecutionPolicy Bypass -File .\tools\bench\run-bench.ps1 -Action RunCase -CaseId sync_1_to_64_core_dense -SavePath run/saves/rl-bench -RconHost 127.0.0.1 -RconPort 25575 -RconPassword redstonelink-bench -AsPlayer BenchBot`
- 当前内置样例：
...
2. `first-mandatory.json`：当前首批必跑，仍可直接使用 `run-first-mandatory-suite*.ps1` 两个包装脚本。
3. `functional-regression.json`：功能回归，覆盖 F02/F03/F04。
4. `restart-crosschunk.json`：跨区块与重启专项，覆盖 X01/X04/R02。
5. `performance-baseline.json`：性能基线，覆盖 lite 与正式性能场景。
6. `nightly-full.json`：夜间全量，覆盖当前全部功能、跨区块/重启与性能场景。
...
2. 日常回归优先跑 `first-mandatory.json` 或 `functional-regression.json`。
3. 改动跨区块或持久化逻辑时补跑 `restart-crosschunk.json`。
4. 做性能采样或对比时跑 `performance-baseline.json`。
5. 夜间巡检或较大版本收口时跑 `nightly-full.json`。
- 输出说明：
