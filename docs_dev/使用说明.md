...
3. bench datapack：`tools/bench/datapack/rl_bench`
4. dedicated server RCON 示例：`tools/bench/bench.server.properties.example`
- 前置条件：
...
6. spark 结果不再依赖 RCON 同步回显，脚本会在发出 spark 命令后轮询 `activity.json`。默认路径推导规则为：若 `SavePath` 位于 `.../saves/<world>`，则优先查 `../spark/activity.json`；否则优先查 `SavePath` 父目录下的 `spark/activity.json`，并回退尝试 `config/spark/activity.json`、`plugins/spark/activity.json`。
