...
4. crosschunk 压力一期矩阵：`tools/bench/matrix-crosschunk-stress.json`
...
4. `tools/bench/matrix-crosschunk-stress.json` 先覆盖 `sync` 持久队列在目标 reload 时的 `1024 pending` 集中释放与空白对照；该矩阵默认关闭 case 自带 chunk 预加载，并允许在 `drive.steps` 中混合 `wait_ticks/command_assert` 时间线。
...
10. `crosschunk-stress-sync.json`：crosschunk 压力一期，先跑 `sync_queue_release_1024_single_shot` 与 `sync_queue_release_1024_blank_control`。
11. `nightly-full.json`：夜间全量，覆盖当前全部功能、跨区块/重启与纯性能基线场景；物理世界端到端场景单独运行。
