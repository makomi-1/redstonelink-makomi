...
7. 命中 `triggerSource` 时，正式 apply 会校验该来源自己的 `sourceRevision`；命中 `core` 时，会校验全图级 `graphRevision`。
...
15. `bench/internal` 现新增 OCC 命令，可直接读取基线并回放 stale baseline：`/redstonelink bench occ snapshot triggerSource <serial>`、`/redstonelink bench occ snapshot core <serial>`、`/redstonelink bench occ pairing submit ... expectedSourceRevision=<n>|expectedGraphRevision=<n>`、`/redstonelink bench occ quick_link apply ... expectedSourceRevision=<n>|expectedGraphRevision=<n>`；命令统一返回 `[RedstoneLink/Bench] occ_*` 机器可读摘要，冲突场景会输出 `outcome=conflict` 而不是命令失败。
...
15. `command_assert` 新增可选 `captureRegex`：对标准化响应执行一次 regex match，并把命名组写入 `phaseResult.captures.*`；后续 phase 可直接引用 `{{phase:<name>.captures.<field>}}`，适合 OCC baseline 先读 `graphRevision/sourceRevision`、下一阶段再提交 stale baseline。
