...
- 隐私名单命令（`/redstonelink privacy current_links mask`）：
...
- 受控名单命令（`/redstonelink write_control protected`）：
...
## 命令/GUI 批量序号输入格式
- 适用入口：`/redstonelink link set ... <targets>`、`/redstonelink activate triggerSource <source_serials> [toggle|pulse]` 与配对 GUI 输入框。
- 统一分隔符：`/`。
