...
10. `crosschunk.directBatching`：loaded direct 链路的批提交模式。`off`=loaded direct `sync/toggle/pulse` 全部 immediate，`queued_only`=仅异步 loaded `sync` 进批提交，`all_direct`=loaded direct `sync/toggle/pulse` 与异步 loaded `sync` 统一进目标级批提交；默认 `all_direct`。兼容读取旧键 `crosschunk.directSyncBatching` 与旧值 `all_sync`。
...
20. 兼容读取：旧键 `crosschunk.triggerSourceChunkUnloadInvalidation.enabled` 仍可读取并映射到 `crosschunk.triggerSourceContextDetachInvalidation.enabled`。
21. `crosschunk.forceLoad.enabled`：强制加载总开关。
22. `crosschunk.forceLoad.mode`：`all` / `whitelist`。
23. `crosschunk.forceLoad.ticketTicks`：强制加载票据时长（tick）。
24. `crosschunk.forceLoad.maxPerTick`：每 tick 强制加载上限。
25. `crosschunk.forceLoad.maxPerSourcePerTick`：每来源每 tick 强制加载上限。
26. `crosschunk.whitelist.sourceTypes` / `crosschunk.whitelist.targetTypes`：可参与白名单的类型。
27. `crosschunk.preset.<name>.sources` / `crosschunk.preset.<name>.targets`：只读 preset（`type:serial`）。
- 组合矩阵（未加载目标时）：
