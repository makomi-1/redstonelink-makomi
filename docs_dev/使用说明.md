...
## 跨区块接管提示（2026-03-13）
- 触发端在“强加载接管实际生效”时提示；relay 中继转发不提示。
...
1. `crosschunk.relay.enabled`：跨区块持久队列主路径总开关（命名沿用 relay）。
2. `crosschunk.relayExpireTicks`：持久队列通用过期时长（tick）。
...
6. `crosschunk.forceLoad.enabled`：强制加载总开关。
7. `crosschunk.forceLoad.mode`：`all` / `whitelist`。
8. `crosschunk.forceLoad.ticketTicks`：强制加载票据时长（tick）。
9. `crosschunk.forceLoad.maxPerTick`：每 tick 强制加载上限。
10. `crosschunk.forceLoad.maxPerSourcePerTick`：每来源每 tick 强制加载上限。
11. `crosschunk.whitelist.sourceTypes` / `crosschunk.whitelist.targetTypes`：可参与白名单的类型。
12. `crosschunk.preset.<name>.sources` / `crosschunk.preset.<name>.targets`：只读 preset（`type:serial`）。
- 组合矩阵（未加载目标时）：
1. `relay=true` + `forceLoad=false`：仅持久队列缓冲（重启后可续跑）。
2. `relay=true` + `forceLoad=true` + `mode=whitelist`：持久队列 + 白名单强制加载。
3. `relay=true` + `forceLoad=true` + `mode=all`：持久队列 + 全量强制加载。
...
8. `sync` 默认按“最新状态”不限时持久化（`crosschunk.syncSignalPersistent=true`），`pulse/toggle` 仍按 TTL 防积压回放。
