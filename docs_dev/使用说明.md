...
6. 站立右键现在会先向服务端请求一次目标节点的 revision 基线，再自动继续正式 apply；用户侧仍表现为一次点击。
7. 命中 `triggerSource` 时，正式 apply 会校验该来源自己的 `sourceRevision`；命中 `core` 时，会校验该目标自己的 `coreRevision`。
8. 若预检与正式 apply 之间目标连接已被其他操作修改，本次 apply 会被拒绝并返回版本冲突提示，不会静默覆盖。
- 清空规则：
1. 中键清空只清空序号缓存与频道缓存。
...
- 基础交互：
1. 主手持有并右键：打开状态面板。
2. 面板内提供 `订阅 / 刷新 / 录制 / 清空` 四个动作按钮；`录制` 当前仅预留入口，点击会直接提示“后续开放”。
...
15. `bench/internal` 现新增 OCC 命令，可直接读取基线并回放 stale baseline：`/redstonelink bench occ snapshot triggerSource <serial>`、`/redstonelink bench occ snapshot core <serial>`、`/redstonelink bench occ pairing submit ... expectedSourceRevision=<n>|expectedCoreRevision=<n>`、`/redstonelink bench occ quick_link apply ... expectedSourceRevision=<n>|expectedCoreRevision=<n>`；其中 `expectedGraphRevision` 仍兼容为旧输入别名，但新文档、新摘要字段统一使用 `expectedCoreRevision/coreRevision`；命令统一返回 `[RedstoneLink/Bench] occ_*` 机器可读摘要，冲突场景会输出 `outcome=conflict` 而不是命令失败。
16. `-PlayerSetupCommands` 会在玩家就绪后、正式 `place/link/input start` 前，以相同玩家上下文依次执行；适合做 `tag/gamemode/tp` 等初始化。该参数只负责“等待并初始化已登录玩家”，不负责启动客户端或自动登录 bot。
17. `-AutoTeleportPlayerToObservationPoint` 为默认关闭的可选开关；启用后，bench 会在当前 case 的 `triggerSource/core` 放置与建链完成后，把玩家自动传送到主要结构最小轴向包围盒顶面中心上方，并朝向结构中心。该开关要求同时提供 `-AsPlayer`。
18. 当提供 `-AutoStartBenchClient` 时，bench 会先在外部客户端实例目录下写入 `config/redstonelink-bench-client.properties`，再启动客户端；客户端内的 RedstoneLink 会读取该文件并自动连接 `-BenchClientGameHost:-BenchClientGamePort`，掉线后按 `-BenchClientReconnectIntervalMs` 自动重连。当前默认 `BenchClientInitialConnectDelayMs=0`、`BenchClientReconnectIntervalMs=1000`。
19. `-AutoStartBenchClient` 第一版要求 `-BenchClientPlayerName` 是明确玩家名；若同时传 `-AsPlayer`，两者必须完全一致，暂不支持把复杂选择器当作自动启动客户端的主身份。
20. `-BenchClientCommandBridgeDispatchMode` 用于指定 bench 玩家命令桥的分发模式：`server_network` 保持“客户端发自定义 payload，服务端代执行”的兼容路径；`client_direct_command` 则让客户端直接调用原生命令发送链，并通过客户端收到的系统消息回填 response 文件。权限专项或需要逼近真实玩家口径时，优先使用 `client_direct_command`。
21. `-BenchClientInstanceRoot` 用于定位外部客户端实例的 `mods`、`config` 等目录；`-BenchClientStartCommand` 既可以直接启动实际游戏进程，也可以是 Prism 这类启动器命令。若使用启动器，bench 会按 `BenchClientInstanceRoot/BenchClientWorkingDirectory` 自动识别后续真正的 `java/javaw` 客户端进程并改为托管它。
22. Prism 一类启动器常会先起 `prismlauncher.exe`，随后再分叉真实 `javaw.exe`，而且其 Java 命令行可能只包含实例目录上一级（如 `...\instances\1.21.1\`）而不直接包含 `...\minecraft\`；bench 现已兼容这一启动/识别时序。
23. 若 Prism 启动阶段的认证/准备较慢，bench 会延长真实客户端进程发现窗口；若短时间内仍拿不到 `tracked pid`，但实例 `logs/latest.log` 已出现新的启动输出，则会先把会话保留为“已观察到启动证据”，后续继续通过玩家 ready 与收尾阶段的进程重发现完成验证，不再把“客户端已实际拉起”误判成启动失败。对带 `--server` 的 launcher auto-join 场景，bench 还会把最小初始连接延迟抬到 `20000ms`，尽量避开 case 入口阶段的 `reload/prepare` 时序。
24. `-SyncLatestClientModJar` 会把当前仓库构建出的最新运行 jar 同步到外部客户端的 `mods` 目录；单 case 只同步客户端，不会热替换已经运行中的 dedicated server mod。
25. 若同时需要 dedicated server 与外部客户端严格使用同一份 jar，推荐在 suite 中同时启用 `-SyncLatestModJar` 与 `-SyncLatestClientModJar`，再用 `-BuildBeforeSyncLatestModJar` 只构建一次。
26. `pulse/toggle` 的纯性能输入器压测会复用 emitter 自身边沿语义；若希望结果口径稳定，建议保持 `server.emitterEdgeMode=rising`。
27. spark 结果不再依赖 RCON 同步回显，脚本会在发出 spark 命令后轮询 `activity.json`。默认路径推导规则为：若 `SavePath` 位于 `.../saves/<world>` 或 `.../rl-cases/<world>`，则优先查 server root 下的 `spark/activity.json`，并回退尝试 `config/spark/activity.json`、`plugins/spark/activity.json`；其它路径则优先查 `SavePath` 父目录下的 `spark/activity.json`。
28. 若服务端 spark 活动文件不在上述默认位置，可显式传入 `-SparkActivityPath <绝对路径>`；若上传较慢，可用 `-SparkActivityTimeoutMs` 调大等待时间。
29. `-BenchClientFocusWindow` 会在 bench 识别出真实客户端进程后尽力把该窗口切到前台；若被系统前台策略拒绝，只记录结果，不会让 case 失败。
30. `-BenchClientOpenTickCharts` 会让客户端在进服稳定后幂等打开 `F3+2` 对应的 tick 曲线；配套延迟可用 `-BenchClientPostJoinActionDelayMs` 调整，默认 `1000ms`。
...
15. `command_assert` 新增可选 `captureRegex`：对标准化响应执行一次 regex match，并把命名组写入 `phaseResult.captures.*`；后续 phase 可直接引用 `{{phase:<name>.captures.<field>}}`，适合 OCC baseline 先读 `graphRevision/sourceRevision/coreRevision`、下一阶段再提交 stale baseline。
16. phase 若需要一次引用多个 serial 集合，可使用 `serialRefs: ["refA", "refB"]`；该写法与旧的 `serialRef` / `sourceGroup` 二选一，不混写。
...
1. 推荐把功能/生命周期回归统一收敛到 4 类：
   ...
   - `occ-conflict-regression`：pairing / quick-link 的 stale baseline 冲突专项。
2. 对应 suite 文件：
   ...
   - `tools/bench/suites/occ-conflict-regression.json`
3. 对应包装脚本：
...
1. 打开配对界面时，服务端当前连接快照会同时携带 `graphRevision/sourceRevision/coreRevision`。
2. `triggerSource` 视角提交时，服务端会校验该来源自己的 `sourceRevision`。
3. `core` 视角提交时，服务端会校验该 `core` 自己的 `coreRevision`，避免多个来源覆盖同一 `core` 时静默互相覆盖；`graphRevision` 仅保留为诊断上下文。
- 冲突反馈规则：
...
19. `triggerSource` 的 `hard invalidation` 固定开启，不再提供独立配置项；来源离线/解绑/退役/删除等非 context-detach 失效仍会剔除目标上的 `toggle/pulse/sync` 贡献。
...
8. `sync` 默认不使用不限时持久化兜底（`crosschunk.syncSignalPersistent=false`）；未加载目标上的常规恢复主链是目标区块加载时的补发（`crosschunk.syncTargetChunkLoadReplay.enabled=true`）。
