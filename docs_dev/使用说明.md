...
- 命令根：
1. `/redstonelink node trace mount <type> <serial> [every] [capacity]`
2. `/redstonelink node trace latest <type> <serial>`
3. `/redstonelink node trace read <type> <serial> [limit]`
4. `/redstonelink node trace unmount <type> <serial>`
5. `/redstonelink node trace list`
- 参数说明：
1. `type` 统一使用 `triggerSource|core`。
2. `serial` 必须是已分配且未退役序号。
3. `every` 为采样周期，单位 tick，默认 `1`，最大 `1200`。
4. `capacity` 为 ring buffer 容量，默认 `128`，最大 `4096`。
5. `limit` 为读取最近样本条数，默认 `10`，最大 `256`。
- 当前支持范围：
1. `core`：支持在线/离线快照。
2. `triggerSource`：首版仅支持当前在线可识别的 `sync triggerSource`；挂载后即使暂时离线，也可继续输出离线快照。
- 行为说明：
1. `mount` 会立即写入一条当前快照，不需要等待下一个 tick 才有第一条样本。
2. 历史样本按“最新优先”读取。
3. 采样时间粒度首版按服务器 `tick` 记录，`slot` 固定为 `0`。
4. `list` 仅展示当前服务端进程内已挂载采样器；服务端重启后需重新挂载。
- 示例：
```powershell
...
/redstonelink node trace mount triggerSource 202 4 128
...
2. `square` 的 `period_ticks` 为完整周期；`high_ticks` 为高电平持续 tick 数。
3. `custom` 的 `sequence` 支持两类格式：
   `0101` / `f0f0` 这类逐 tick 位串/十六进制串。
   `15/0/7/0` 这类显式功率列表。
4. `phase_ticks` 用于整体相位偏移。
5. `total_ticks=0` 表示无限持续，直到手动 `stop` 或 `clear`。
- 常用示例：
```powershell
