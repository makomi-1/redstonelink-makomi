...
## 高并发大型核心红石粉路网注意事项
- 不推荐在同一时间高并发激活大型核心红石粉路网（尤其顶面参与线网传播的场景），可能出现瞬时高延迟。
- 可通过增大 `server.topActiveSyncThrottleTicks` 缓解更新压力（默认 `2`，可按压测结果逐步上调）。
- 调大节流窗口后，`active` 客户端观测态（如 F3）同步会更慢，但不改变红石路网 `power` 计算语义。

## 顶面核心红石粉 ACTIVE 同步节流窗口
- 配置项：`server.topActiveSyncThrottleTicks`
- 默认值：`2`
- 取值范围：`1~40`（超出范围会自动夹紧）
- 作用范围：仅影响顶面核心红石粉 `active` 的客户端同步频率（F3 可观测态），不影响红石路网 `power` 计算。
