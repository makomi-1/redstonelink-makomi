 ...
 * 世界外显穿透几何公共渲染支持。
 * <p>
 * 26.1 现有 level 渲染事件下，普通线框和调试面层仍会受深度测试影响，
 * 本模组的连线、描边与定向箭头需要统一保持“穿墙可见”语义，
...
final class SeeThroughWorldGeometryRenderSupport {
	 ...
	 * 提交并立即绘制一批穿透世界遮挡的线段。
	 */
	static void renderLines(LevelRenderContext worldRenderContext, List<ColoredLineSegment> lineSegments) {
		if (lineSegments == null || lineSegments.isEmpty()) {
			return;
		}
		Minecraft minecraft = Minecraft.getInstance();
		if (minecraft == null || minecraft.getMainRenderTarget() == null || worldRenderContext == null || worldRenderContext.poseStack() == null) {
			return;
		}
		...
		finishAndDraw(
			...
			ringBuffer -> lineVertexBuffer = ringBuffer,
			lineVertexBuffer
		);
	 ...
	 */
	static void renderFilledQuads(LevelRenderContext worldRenderContext, List<ColoredQuad> quads) {
		...
		finishAndDraw(
			...
			ringBuffer -> filledVertexBuffer = ringBuffer,
			filledVertexBuffer
		);
