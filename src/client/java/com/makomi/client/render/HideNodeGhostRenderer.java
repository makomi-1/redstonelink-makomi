 ...
 * 隐藏节点幽灵模型渲染器。
 * <p>
 * 仅在佩戴智能眼镜时，为隐藏节点补一层半透明普通模型，并绘制轮廓框。
 * </p>
 */
public final class HideNodeGhostRenderer<T extends PairableNodeBlockEntity> implements BlockEntityRenderer<T> {
	...
	private static final float OUTLINE_ALPHA = 0.75F;
	...
	public void render(
		...
		if (minecraft.player != null && SmartGlassesAccessSupport.canRenderSerialOverlay(minecraft.player)) {
			BlockState ghostDisplayState = resolveGhostDisplayState(blockEntity.getBlockState());
			if (ghostDisplayState != null) {
				renderGhostModel(minecraft, ghostDisplayState, poseStack, buffer, packedOverlay);
				renderGhostOutline(blockEntity, poseStack, buffer);
			}
		}
	...
	private static void renderGhostOutline(
		...
		poseStack.pushPose();
		LevelRenderer.renderLineBox(poseStack, vertexConsumer, 0.0D, 0.0D, 0.0D, 1.0D, 1.0D, 1.0D, red, green, blue, OUTLINE_ALPHA);
		poseStack.popPose();
	}
